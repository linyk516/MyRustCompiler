...
impl Automaton {
    ...
    pub fn build_canonical_collection(
        ...
        // 构造I0，作为迭代基础
        let initial_item_set = Self::closure_(
            grammar,
            &nullable_set,
            &first_sets,
            &initial_item_set
        ...
        while let Some(item) = queue.pop_front() {
            ...
            for next_symbol in next_symbols {
                ...
                if let Some(new_state_id) = seen.get(&new_item_set).cloned() {
                    // 已出现过，直接添加转移
                    automation.transitions.insert((current_state_id.clone(), next_symbol), new_state_id);
                } else {
                    // 未出现过，加入并添加转移
