...
mod lexer_ident_or_keyword_tests {
    ...
    fn assert_keyword(token: &super::super::token::Token, text: &str) {
        match text {
            ...
            "return" => assert!(matches!(token.kind, TokenKind::Keyword(KeywordKind::Return))),
            ...
            "continue" => assert!(matches!(token.kind, TokenKind::Keyword(KeywordKind::Continue))),
    ...
    fn lex_ident_or_keyword_parses_keyword_let() {
        ...
        let token = lexer.lex_ident_or_keyword().expect("token should be produced");
...
mod lexer_operator_or_assign_tests {
    ...
    fn lex_operator_or_assign_parses_single_char_operators() {
        ...
        for (src, kind) in cases {
            let mut lexer = Lexer::new(src);
            let token = lexer.lex_operator_or_assign().expect("token should be produced");
            assert_eq!(token.span.start, 0);
            assert_eq!(token.span.end, 1);
            match kind {
                OperatorKind::Plus => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Plus))),
                OperatorKind::Minus => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Minus))),
                OperatorKind::Star => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Star))),
                OperatorKind::Slash => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Slash))),
                OperatorKind::Gt => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Gt))),
                OperatorKind::Lt => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Lt))),
                OperatorKind::Amp => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Amp))),
                _ => unreachable!(),
            }
        }
    }
    ...
    fn lex_operator_or_assign_parses_double_char_operators() {
        ...
        for (src, kind) in cases {
            ...
            match kind {
                OperatorKind::EqEq => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::EqEq))),
                OperatorKind::Ge => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Ge))),
                OperatorKind::Le => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Le))),
                OperatorKind::Ne => assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Ne))),
                _ => unreachable!(),
            }
        }
    }
...
mod lexer_delimiter_tests {
    ...
    fn lex_delimiter_parses_all_delimiters() {
        ...
        for (src, kind) in cases {
            ...
            assert_eq!(token.span.end, 1);
            match kind {
                DelimiterKind::LParen => assert!(matches!(token.kind, TokenKind::Delimiter(DelimiterKind::LParen))),
                DelimiterKind::RParen => assert!(matches!(token.kind, TokenKind::Delimiter(DelimiterKind::RParen))),
                DelimiterKind::LBrace => assert!(matches!(token.kind, TokenKind::Delimiter(DelimiterKind::LBrace))),
                DelimiterKind::RBrace => assert!(matches!(token.kind, TokenKind::Delimiter(DelimiterKind::RBrace))),
                DelimiterKind::LBracket => assert!(matches!(token.kind, TokenKind::Delimiter(DelimiterKind::LBracket))),
                DelimiterKind::RBracket => assert!(matches!(token.kind, TokenKind::Delimiter(DelimiterKind::RBracket))),
            }
        }
    }
...
mod lexer_separator_tests {
    ...
    fn lex_separator_parses_all_separators() {
        ...
        for (src, kind) in cases {
            ...
            assert_eq!(token.span.end, 1);
            match kind {
                SeparatorKind::Semicolon => assert!(matches!(token.kind, TokenKind::Separator(SeparatorKind::Semicolon))),
                SeparatorKind::Colon => assert!(matches!(token.kind, TokenKind::Separator(SeparatorKind::Colon))),
                SeparatorKind::Comma => assert!(matches!(token.kind, TokenKind::Separator(SeparatorKind::Comma))),
            }
        }
    }
...
mod lexer_special_tests {
    ...
    fn lex_special_parses_arrow_dot_and_dotdot() {
        ...
        assert!(matches!(token_arrow.kind, TokenKind::Special(SpecialKind::Arrow)));
        ...
        assert!(matches!(token_dot.kind, TokenKind::Special(SpecialKind::Dot)));
        ...
        let token_dotdot = lexer_dotdot.lex_special().expect("token should be produced");
        assert!(matches!(token_dotdot.kind, TokenKind::Special(SpecialKind::DotDot)));
    ...
    fn lex_special_prefers_longest_match_for_dotdot() {
        ...
        assert!(matches!(token.kind, TokenKind::Special(SpecialKind::DotDot)));
        assert_eq!(token.span.start, 0);
        assert_eq!(token.span.end, 2);
...
mod lexer_branch_coverage_tests {
    ...
    fn lex_operator_or_assign_falls_back_to_single_char_on_invalid_two_char_text() {
        ...
        assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Plus)));
        assert_eq!(token.span.start, 0);
        assert_eq!(token.span.end, 1);
        assert_eq!(lexer.cursor.pos(), 1);
        assert_eq!(lexer.cursor.peek(), Some('='));
...
mod lexer_next_token_tests {
    use super::super::token::{KeywordKind, LiteralKind, OperatorKind, TokenKind};
    use super::super::Lexer;
    ...
    fn next_token_minus_falls_back_to_operator() {
        ...
        assert!(matches!(token.kind, TokenKind::Operator(OperatorKind::Minus)));
        assert_eq!(token.span.start, 0);
        assert_eq!(token.span.end, 1);
    }
    ...
    fn next_token_program_like_sequence_matches_expected_kinds() {
        ...
        assert!(matches!(kinds[18], TokenKind::Operator(OperatorKind::Minus)));
